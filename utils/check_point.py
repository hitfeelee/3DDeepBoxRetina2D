...
class Checkpointer():
    ...
    def load(self, f=None, use_latest=True):
        ...
        if getattr(self.cfg['SOLVER'], 'LOAD_SOLVER'):
            if "solver" in checkpoint and self.solver:
                solver_name = self.solver.solver_name
                self.logger.info("Loading solver from {}".format(f))
                cp = checkpoint.pop("solver")
                solver_name_old = 'Adamax'
                if 'solver_name' in cp:
                    solver_name_old = cp.pop('solver_name')
...
class DetectronCheckpointer(Checkpointer):
    def __init__(
        ...
        super(DetectronCheckpointer, self).__init__(
            model, solver, save_dir, save_to_disk, logger
        )
        self.cfg = cfg.clone()
